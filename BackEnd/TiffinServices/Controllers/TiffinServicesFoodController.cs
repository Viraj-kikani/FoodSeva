...
{
    [Area("tiffinservices")]
    public class TiffinServicesFoodController : TiffinServicesBaseController
    {
        private readonly string _imageFolderPath;
        private readonly string _RestImageFolderPath;

        public TiffinServicesFoodController()
        {
            _imageFolderPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Images", "Food");
            Directory.CreateDirectory(_imageFolderPath);
            _RestImageFolderPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Images", "TiffinServices");
        ...
        [Route("tiffinservices/change-tiffinservices-detail")]
        [HttpGet]
        public ActionResult GetTiffinServicesDetailById()
        {
            EditTiffinServicesModel tiffinServicesRegistrationModel = new EditTiffinServicesModel();

            GetTiffinServicesDetailById getTiffinServicesDetailById = objDatabaseTiffinServices.GetTiffinServicesDetailsById(GetCurrentTiffinServices().TiffinServicesID);
            if (getTiffinServicesDetailById != null)
            {
                tiffinServicesRegistrationModel.TiffinServicesID = getTiffinServicesDetailById.TiffinServicesID;
                tiffinServicesRegistrationModel.OwnerName = getTiffinServicesDetailById.OwnerName;
                tiffinServicesRegistrationModel.TiffinServicesName = getTiffinServicesDetailById.TiffinServicesName;
                tiffinServicesRegistrationModel.MobileNo = getTiffinServicesDetailById.MobileNo;
                tiffinServicesRegistrationModel.Email = getTiffinServicesDetailById.Email;
                tiffinServicesRegistrationModel.ShopPlotNumber = getTiffinServicesDetailById.ShopPlotNumber;
                tiffinServicesRegistrationModel.Floor = getTiffinServicesDetailById.Floor;
                tiffinServicesRegistrationModel.BuildingName = getTiffinServicesDetailById.BuildingName;
                tiffinServicesRegistrationModel.ZipCode = getTiffinServicesDetailById.ZipCode;
                tiffinServicesRegistrationModel.TiffinServicesImageName = getTiffinServicesDetailById.TiffinServicesImageName;
            ...
            return View("_EditTiffinServicesDetail", tiffinServicesRegistrationModel);
        }
        [Route("tiffinservices/change-tiffinservices-detail")]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult ChangeTiffinServicesDetail(EditTiffinServicesModel tiffinServicesRegistrationModel, IFormFile ImageName)
        {
            try
            {
                string Result = "";
                ...
                tiffinServicesRegistrationModel.TiffinServicesID = GetCurrentTiffinServices().TiffinServicesID;
                Result = objDatabaseTiffinServices.ChangeTiffinServicesDetail(tiffinServicesRegistrationModel, NewFileName);
                ...
                return RedirectToAction("GetTiffinServicesDetailById");

            }
            catch (Exception ex) { throw; }
        }
        [HttpPost]
        public ActionResult DeleteTiffinServicesImage(string ImageName)
        {
            try
            {
        ...
        [Route("tiffinservices/food-item")]
        ...
        public ActionResult GetFoodItemList(JQueryDataTableParamModel param, string Name, string DiscountInPercentage, string IsAvailable, string IsVegetarian, string IsBestSeller)
        {
            try
            {
                ...
                List<FoodItemListModel> list = objDatabaseTiffinServices.GetFoodItemList(param, Name, DiscountInPercentage, IsAvailable, IsVegetarian, IsBestSeller, GetCurrentTiffinServices().TiffinServicesID, out noOfRecords);
                obj = from c in list
                      select new[]
                      {
        ...
        [Route("tiffinservices/add-edit-food")]
        [HttpGet]
        public ActionResult AddEditFood(int FoodId)
        {
            AddEditFoodViewModel addEditfoodModel = new AddEditFoodViewModel();

            GetFoodDetailsById getPackageDetailsById = objDatabaseTiffinServices.GetFoodItemDetailsById(FoodId, GetCurrentTiffinServices().TiffinServicesID);
        ...
        public IActionResult AddEditFood(AddEditFoodViewModel addEditFoodViewModel, IFormFile ImageName)
        {
            try
            {
                if (addEditFoodViewModel.FoodId > 0)
                {
                    ...
                    addEditFoodResponse = objDatabaseTiffinServices.AddEditFood(addEditFoodViewModel, GetCurrentTiffinServices().TiffinServicesID, NewFileName);
        ...
        public ActionResult DeleteFoodItem(int FoodItemId)
        {
            try
            {
                ...
                DeleteFoodResponse = objDatabaseTiffinServices.DeleteFood(FoodItemId);
        ...
        #region Order
        [Route("tiffinservices/tiffinservices-order")]
        [HttpGet]
        public ActionResult TiffinServicesOrderList()
        {
            return View("_OrderList");
        }
        [HttpGet]
        public ActionResult GetTiffinServicesOrderList(JQueryDataTableParamModel param, string Name)
        {
            try
            {
                IEnumerable<string[]> obj = Enumerable.Empty<string[]>();
                ...
                var SortOrderString = param.sColumns.Split(',');
                param.iSortCol_0 = SortOrderString[Convert.ToInt32(param.iSortCol_0)];
                List<OrderListModel> list = objDatabaseTiffinServices.GetTiffinServicesOrderList(param, Name, GetCurrentTiffinServices().TiffinServicesID, out noOfRecords);
                obj = from c in list
                      select new[]
                      {
        ...
        {
            List<OrderstatusList> list = objDatabaseTiffinServices.GetOrderstatusList();
            return list;
        }
        [Route("tiffinservices/change-order-status")]
        [HttpGet]
        public ActionResult GetOrderDetailByOrderId(int OrderDetailID)
        {
            OrderViewModel orderViewModel = new OrderViewModel();

            orderViewModel = objDatabaseTiffinServices.GetOrderDetailByOrderId(OrderDetailID, GetCurrentTiffinServices().TiffinServicesID);
        ...
        public IActionResult ChangeOrderStatus(OrderViewModel orderViewModel)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    orderStatusResponse = objDatabaseTiffinServices.ChangeOrderStatus(orderViewModel, GetCurrentTiffinServices().TiffinServicesID);
                    return RedirectToAction("TiffinServicesOrderList", new { Status = orderStatusResponse.status });
                }
                else
                {
                    return View("_ChangeOrderStatus", orderViewModel);
